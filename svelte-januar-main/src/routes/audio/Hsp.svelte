...
<section>
  <article class="flex flex-col px-2 py-1 ">
    <!-- SERIEN: idi, name, genre, studio, logo, card, icon, info, op    -->
    {#each data1 as d}
    ...
    <article>
        <ul>
            <!-- Episoden: id, idi, ep, titel, jahr, dauer, buch, cover, extra, links, inhalt, op, name, genre, studio    -->
            {#each data2 as d}
            <ListDouble href="#/hsp/eps/{d.id}" head="{d.titel}" end="{d.ep}" sub="{d.djahr} / {d.dauer}" logo="{d.cover}" />
            {/each}
        </ul>
    </article>
</section>
